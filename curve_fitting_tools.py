...
def fitzgibbon_e_fit(x,y,e):
    ...
    # build design and constraint matrix
    C = np.array([[0,0,2],[0,-1,0],[2,0,0]])
    B = np.array([[e_cal, 0,-e_cal],[0,(2-e**2)**2,0],[-e_cal,0,e_cal]])
    D1 = np.zeros((num,3))
    D2 = np.ones((num,3))
    for i in range(0,num):
        D1[i,0] = x[i]**2
    ...
    M = np.matmul(S2,T)
    M = np.add(S1,M)
    M = np.matmul(np.linalg.pinv(B),M)
    w,V = np.linalg.eig(M)
    ...
    # elif w[2] > 0 and (4*V[0,2]*V[2,2] - V[1,2]**2) > 0:
    #     A1 = V[:,2]
    if (4*V[0,0]*V[2,0] - V[1,0]**2) > 0 :
        A1 = V[:,0]
    elif (4*V[0,1]*V[2,1] - V[1,1]**2) > 0:
        A1 = V[:,1]
    elif (4*V[0,2]*V[2,2] - V[1,2]**2) > 0:
        A1 = V[:,2]
    else:
        print("Fitzgibbon failed")
        exit()
